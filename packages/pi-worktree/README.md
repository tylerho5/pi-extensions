...
- **State is a file, so it survives restarts.** The active session lives at `~/.pi/agent/worktree-state.json` (`WorktreeStateStore`), keyed by the session's own cwd. On `session_start`, state whose `sessionCwd` matches is restored: the session re-enters the worktree and a notify explains how to leave. `/worktree reset` clears desynced state without touching the worktree. State from a *different* directory is ignored, never cleared.
...
- `session_start` — re-registers the seven cwd-aware proxies for this session's cwd; restores persisted state whose `sessionCwd` matches (refreshes the lock pid, notifies); sets/clears the footer status.
