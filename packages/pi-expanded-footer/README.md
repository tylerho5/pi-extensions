...
- **Child-agent cost shows as a separate term.** `shared/child-cost.ts` accumulates USD spent by subagents (pi and claude backends) and workflow children; when nonzero, line 1 renders `$0.041 +$0.123 agents` (main session cost + accumulated child cost). The accumulator resets on `session_start` reasons `new`/`startup` only, so resume/fork keep prior child spend just as the main cost keeps restored history. Read `docs/shared.md` (child-cost) before touching the publishers.
