...
1. **Task-rail widget** (above the editor): header `↗ Subagents · counts · hints`
   plus up to 8 rows when expanded; removed the former footer status line
   (`subagents: ■ N running · ...`) as it duplicated the rail's counts.
