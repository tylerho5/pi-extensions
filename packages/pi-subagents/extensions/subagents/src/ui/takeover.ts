...
export async function openSubagentPicker(
  ...
  while (true) {
    // The dashboard is for model-origin subagents only; btw asides have
    // their own panel on /btw.
    if (view.list().filter(isModelVisible).length === 0) {
...
class SubagentDashboard implements Component {
  ...
  private subs(): ReadonlyArray<SubagentSnapshot> {
    return this.view.list().filter(isModelVisible);
  }
...
class TakeoverView implements Component, Focusable {
  ...
  constructor(
    ...
    this.unsubscribe = view.subscribeTo(id, () => {
      // The live transcript always renders (mergeTranscripts keeps it
      // current), so nothing vanishes while streaming. The persisted one is a
      // snapshot: reload it once when the run settles to recover full tool
      // outputs and any items pruned from the bounded live buffer.
      const snap = this.snap();
      if (snap && snap.status !== "running") this.loadFullTranscript(true);
      else this.loadFullTranscript();
      this.scheduleRender();
    });
  ...
  private loadFullTranscript(force = false) {
    if (this.loadingTranscript) return;
    if (!force && this.persistedTranscript) return;
    const path = this.snap()?.meta.sessionFilePath;
    if (!path) return;
    this.loadingTranscript = true;
    void loadPersistedTranscript(path)
      .then((transcript) => {
        if (this.closed) return;
        this.persistedTranscript = transcript;
        this.loadingTranscript = false;
        this.transcriptError = undefined;
        // The run settled while this load was in flight and its settle-time
        // reload was blocked by loadingTranscript; reload once more to
        // capture the final state.
        if (!force && this.snap()?.status !== "running") {
          this.loadFullTranscript(true);
          return;
        }
        this.scheduleRender();
      })
  ...
  render(width: number): string[] {
    ...
    const transcript = buildTranscriptLines(
      {
        ...snap,
        transcript: mergeTranscripts(this.persistedTranscript, snap.transcript),
      },
      width,
      theme,
    ...
    if (this.loadingTranscript) {
      body.push(theme.fg("dim", "loading full persisted transcript…"));
    }
    ...
    lines.push(
      truncateToWidth(
        theme.fg(
          "dim",
          `${configuredKeys(this.keybindings, "tui.input.submit")} send · ${configuredKeys(this.keybindings, "app.interrupt")} back · ${configuredKeys(this.keybindings, "app.thinking.toggle")} thinking · ${configuredKeys(this.keybindings, "app.tools.expand")} tools · ${configuredKeys(this.keybindings, "app.clear")} abort run · ${configuredKeys(this.keybindings, "tui.editor.cursorUp")}/${configuredKeys(this.keybindings, "tui.editor.cursorDown")} scroll · ${configuredKeys(this.keybindings, "tui.editor.pageUp")}/${configuredKeys(this.keybindings, "tui.editor.pageDown")} page`,
        ),
        width,
      ),
    );
