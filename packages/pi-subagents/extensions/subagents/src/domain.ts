...
export interface ParentContext {
  ...
  /**
   * Parent conversation at spawn time (btw only). The pi backend seeds the
   * child session file with these messages so the child inherits the
   * conversation; model-origin subagents stay isolated.
   */
  readonly contextMessages?: SessionContext["messages"];
}
...
export type TranscriptItem =
  ...
  | {
      ...
      /**
       * Full multi-line tool output, set only by the persisted-transcript
       * loader (`src/persisted/`). The live pipeline keeps just the
       * single-line `outputPreview`; this field recovers the rest from disk.
       */
