...
export const SUBAGENT_SPAWN_PROMPT_GUIDELINES = [
  ...
  "Omit `model` and `reasoning_effort` unless the user named a model. Both harnesses default to the user's configured subagent models, and a cost ceiling rejects expensive models you pick yourself.",
...
export const SUBAGENT_SPAWN_PARAMETER_DESCRIPTIONS = {
  ...
  model:
    'Model hint, interpreted by the chosen harness (pi: "provider/model-id" or model id; claude: an alias like "sonnet"). Omit unless the user named a model: each harness uses the user\'s configured subagent model, and models above the cost ceiling are rejected.',
  reasoningEffort:
    "Reasoning effort on a shared scale; the harness maps it to its nearest native equivalent (pi thinking level, claude thinking budget). Omit for the configured default.",
};
...
export function buildSubagentSpawnResult(options: {
  ...
  return (
    `Spawned ${options.id} "${options.title}" (${options.harness}, ${options.modelLabel}). ` +
    `Runs in background; result arrives automatically. subagent_wait(["${options.id}"]) to block.`
  );
}
...
/**
 * Builds the child completion/failure wrapper injected into the parent
 * model's context. It carries only a preview of the output: the full text is
 * rendered in the session entry (via details.fullOutput) and retrievable with
 * subagent_wait, so the parent model never re-dumps it into the transcript.
 */
export function buildSubagentResultMessage(options: {
  ...
  text += `\n\n[Preview of the ${options.sizeLabel} output — the full result is shown to the user in the session. Do not repeat this output in your reply. Call subagent_wait(["${options.id}"]) if you need the complete output.]`;
  return text;
}
