...
export interface AgentRecord {
  ...
  startedAt: number;
  finishedAt?: number;
  /** Replayed from a prior run's journal rather than executed. */
  cached?: boolean;
  /** Retained git worktree path when the agent left uncommitted changes. */
  worktree?: string;
  error?: string;
  preview: string;
  usage: AgentUsage;
...
export interface WorkflowDetails {
  ...
  currentPhase?: string;
  agents: AgentRecord[];
  /** Script-emitted log() lines, including pipeline stage failures. */
  logs?: string[];
  result?: unknown;
...
/** "alias→resolved-id" when the requested name needed mapping, else the id. */
export function agentModelLabel(agent: {
  model?: string;
  requestedModel?: string;
}): string | undefined {
  if (!agent.model) return agent.requestedModel;
  return agent.requestedModel && agent.requestedModel !== agent.model
    ? `${agent.requestedModel}→${agent.model}`
    : agent.model;
}
