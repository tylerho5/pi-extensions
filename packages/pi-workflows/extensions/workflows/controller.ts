...
/** Claude Code's fan-out formula, so a pi workflow schedules like a CC one. */
export const DEFAULT_CONCURRENCY = Math.min(
  16,
  Math.max(2, os.cpus().length - 2),
);
/** Runaway-loop backstop, set far above any real workflow (CC's value). */
export const MAX_AGENT_CALLS = 1000;
