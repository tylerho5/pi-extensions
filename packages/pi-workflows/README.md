...
- **model.ts** — `WorkflowDetails`, `AgentRecord`, `AgentUsage` (`emptyUsage`), `TranscriptEntry`, state/status helpers `stateSquare`, `statusSquare`, `statusWord`, `statusColor`, `SQUARE`, formatting `formatElapsed`, `formatUsage`, `formatTokens`, `shortenHome`, `agentContext`, `aggregateUsage`, `countStates`, `phaseGroups`, `resultJson`, constants `RESULT_JSON_MAX_BYTES`/`RESULT_JSON_MAX_LINES`.
