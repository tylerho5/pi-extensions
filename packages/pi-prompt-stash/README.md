...
`pi.registerShortcut("ctrl+s", { description: "Stash the current prompt draft, or restore the stashed draft", handler: toggleStash })`. Handler signature `(ctx: ExtensionContext) => Promise<void> | void` (pi's declared type; `toggleStash` is `async`); fires from the default editor's key handling (editor-scoped), and `ctrl+s` is not in pi's reserved-for-extensions conflict list.

### Command: `/stash`

`pi.registerCommand("stash", { description: "Stash the current prompt draft, or restore the stashed draft (same as ctrl+s)", handler: async (_args, ctx) => toggleStash(ctx) })`. Args ignored; identical behavior to the shortcut. Useful when `ctrl+s` is captured by a picker-scoped builtin.
...
1. **Stash a half-written prompt.** Mid-draft, the user wants to run something else: `ctrl+s` saves the draft (raw, untrimmed), clears the editor, and the footer shows `prompt-stash: stashed: <first ~40 chars, whitespace-squashed>…`. Later, with an empty editor, `ctrl+s` restores it byte-for-byte and shows a "Draft restored" notification.
...
3. **Overwrite, not stack.** Draft A is stashed; the user types draft B and hits `ctrl+s` again — A is gone, B occupies the single slot. Restore yields B.
4. **No-op cases.** Empty (or whitespace-only) editor with no stash: `ctrl+s` does nothing — no status change, no notification. Whitespace-only editor text *with* a stash counts as "empty" and restores.
