...
3. **Consolidate with `/dream`** — after a week of sessions, run `/dream`: it skips gates 4–7 but takes the lock, spawns the confined child (`deepseek/deepseek-v4-pro`, effort max), reads redacted session excerpts (mains first), and notifies `✦ Dream consolidated 3 memory files · $0.12 · 48.2k tok` with the dream's summary. `/dream-log` shows the full history (statuses, reasons, costs); the lock's mtime now blocks idle dreams for 24 h.
