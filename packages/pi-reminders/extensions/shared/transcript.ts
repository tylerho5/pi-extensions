...
/**
 * Holds the oldest un-recapped baseline. Recaps are deferred until the session
 * goes quiet, so several runs can accumulate behind one; keeping the first
 * baseline makes that recap cover all of them instead of only the last.
 */
export function createRunBoundary() {
  let marker: RunMarker | undefined;

  return {
    begin(baselineLeafId: string | null) {
      marker ??= { baselineLeafId };
    },
    pending() {
      return marker;
    },
