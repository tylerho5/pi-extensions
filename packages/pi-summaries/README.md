...
- `config.private.json` (`~/.pi/agent/summaries/config.private.json`, gitignored, mode `0o600`): `{ "enabled", "provider", "model", "reasoning" }`. `parseSummaryConfig` falls back to `DEFAULT_SUMMARY_CONFIG` (`enabled: true`, `deepseek` / `deepseek-v4-flash` / `medium`) on any missing/invalid field, except a missing `enabled`, which means `true` (configs written before the toggle predate the field; a mistyped `enabled` still falls back to defaults). Config used to live inside the extension directory (`extensions/summaries/config.private.json`), which broke on a clean package install; that old file is now ignored — users re-select their summary model once via `/recap`.
