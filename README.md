...
| [`@tylerho/pi-ask-user-question`](packages/pi-ask-user-question) | published 0.2.1 | Claude Code-shaped batch questions: 1-4 per call, each with a header, 2-4 options, optional multi-select and per-option previews. |
...
| [`@tylerho/pi-expanded-footer`](packages/pi-expanded-footer) | published 0.2.1 | Replaces the default footer with a Claude Code-style layout: model, context usage, cost, branch, worktree. |
...
| [`@tylerho/pi-worktree`](packages/pi-worktree) | published 0.2.1 | Lets the agent create, enter, and exit isolated git worktrees for parallel work in one repo. |
